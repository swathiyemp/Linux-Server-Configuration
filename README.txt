...
2.       Then type sudo ufw default deny incoming to block all incoming requests
3.       Then type sudo ufw default allow outgoing  to allow all outgoing traffic
4.       Then type in sudo ufw allow 2200/tcp  # SSH
5.       sudo ufw allow 80/tcp  # HTTP
6.       sudo ufw allow 123/udp  # NTP
7.       Enable the firewall by sudo ufw enable.
8.         Check the status by sudo ufw status.
9.        In the networking tab of lightsail Instance perform the following steps.
10.        Update ssh with custom tcp 2200
11.        Also add custom udp 123.
12.        The server gets disconnected.Login from git bash with the following command--ssh ubuntu@34.232.46.54 -p 2200 -i ~/.ssh/Lightsail Default Private Key.pem
...
2. Confirm it by going here--http://34.232.46.54:80
3. Install the mod_wsgi by typing in sudo apt-get install libapache2-mod-wsgi-py
4. Create a new directory for storing our web application by typing in sudo mkdir /var/www/itemcatalog
5. Change the ownership by typing in sudo chown -R ubuntu /var/www/itemcatalog
6. Create a new file itemcatalog.wsgi by typing in nano /var/www/itemcatalog/itemcatalog.wsgi
7. Enter the following into the file and save it.
             ...
             return [output]
      8.   Create the configuration file by typing sudo nano /etc/apache2/sites-available/itemcatalog.conf and tye in the following
...
<VirtualHost *:80>
  ...
  CustomLog ${APACHE_LOG_DIR}/access.log combined
   ...
   9.   Enable the virtual host by typing in sudo a2ensite itemcatalog
  10.  Restart the server by typing in sudo apache2ctl restart
